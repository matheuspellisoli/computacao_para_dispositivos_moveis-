...
public class TarefaDAO {
    ...
    public static void editar(Task task, Context context){
        ...
        db.update("tarefa", valores, " id = " + task.id , null );
    }

    public static void excluir(int id, Context context){
        Banco banco = new Banco(context);
        SQLiteDatabase db = banco.getWritableDatabase();
        db.delete("tarefa", " id = " + id , null);
    }

    public static List<Task> getTarefas(Context context){
        List<Task> lista = new ArrayList<>();
        Banco banco = new Banco(context);
        SQLiteDatabase db = banco.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT id, titulo, obs, dateTime FROM tarefa ORDER BY titulo", null );
